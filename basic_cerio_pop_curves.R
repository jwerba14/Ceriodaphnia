...
## put days 49 and 50 together -- no rep was counted on both/ just based on start/end day

dat$Day <- ifelse(dat$Day == 49, 50, dat$Day)

sumd <- dat %>% 
  group_by(Day, Treatment) %>%
  filter(!is.na(Population)) %>%
  summarize(med = unlist(quantile(Population, c(0.5))), 
            upr = unlist(quantile(Population, c(0.975))), 
            lwr = unlist(quantile(Population, c(0.025))),
            repl = length(unique(Rep)))



sumd$int <- interaction( as.factor(sumd$Treatment), as.factor(sumd$Day))

ggplot(sumd, aes(int, med)) + geom_point(aes(color = as.factor(Treatment), size = repl)) + 
  geom_errorbar(aes(ymin = lwr, ymax=upr, color = as.factor(Treatment)), width = 0.2) +
  scale_x_discrete(breaks = c(25.1,25.8,25.15,25.22,25.29,25.36,25.43, 10.50),
                 labels= c(1,8,15,25,29,36,43,50)) + ylab("Ceriodaphnia per 50ml") + xlab("Day")+
  scale_color_discrete("Treatment", labels = c("3 ug/L Chl-a", "10 ug/L Chl-a", "22 ug/L Chl-a", "65 ug/L Chl-a"))+
  scale_size_continuous("Number of Replicates") + theme(legend.position = c(0.15, 0.8))
